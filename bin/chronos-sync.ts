...
switch (cmd) {
    ...
    if (args.includes('--status')) {
      import('../src/state-file.js').then(async (m) => {
        ...
        process.stdout.write(
          JSON.stringify({ started_at: started, last_cycle_at: lastCycle, rooms: state.rooms }, null, 2) + '\n'
        )
      }).catch((err: unknown) => {
      ...
      process.stderr.write(
        '\x1b[33m!\x1b[0m "chronos-sync daemon"은 launchd 호환용으로만 유지됩니다. ' +
          '터미널에서 직접 사용하실 때는 인자 없이 "chronos-sync"를 실행하세요.\n'
      )
      import('../src/daemon.js').then((m) => m.main()).catch((err: unknown) => {
  ...
  case 'status':
    import('../src/cli/status.js').then((m) => m.runStatus()).catch((err: unknown) => {
      process.stderr.write('chronos-sync: status error: ' + String(err) + '\n')
      process.exit(1)
    })
    break
    ...
    .then(([daemon, ui]) => {
      ...
      return import('../src/state-file.js').then(async ({ loadConfig }) => {
        ...
        try {
          ...
          process.stderr.write(
            'chronos-sync: config load error: ' +
