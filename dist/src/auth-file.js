 ...
 */
export function chronosHomeDir() {
...
export function bootstrapCachePath() {
 ...
 */
export async function ensureChronosDir() {
...
export async function loadAuth() {
    ...
    try {
        ...
        throw new Error(`auth.json is not valid JSON: ${e.message}`);
    }
 ...
 */
export async function loadPatFile() {
    try {
 ...
 */
export async function savePatFile(token) {
    ...
    if (mode !== 0o600) {
        throw new Error(`${path} mode is ${mode.toString(8)} after write — expected 600. ` +
...
function validateAuth(value) {
    ...
    const v = value;
    for (const k of ['server_url', 'user_email', 'pat_hash_prefix', 'written_at']) {
        if (typeof v[k] !== 'string' || v[k].length === 0) {
    ...
    return {
        ...
        pat_hash_prefix: v.pat_hash_prefix,
        pat_storage: v.pat_storage,
