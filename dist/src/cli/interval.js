...
import { getSyncSettings, putSyncSettings, ApiPatAuthError, } from '../api-client.js';
 ...
 */
export async function runIntervalSet(rawSeconds, out = process.stdout, err = process.stderr) {
    ...
    if (parsed < MIN_INTERVAL_SECONDS || parsed > MAX_INTERVAL_SECONDS) {
        err.write(`값 범위 오류: ${parsed}초 — ${MIN_INTERVAL_SECONDS}~${MAX_INTERVAL_SECONDS}초 사이여야 합니다.\n`);
        return { exitCode: 1 };
    }
    ...
    let response;
    try {
        response = await putSyncSettings({ serverUrl: config.server_url, pat: config.pat }, parsed);
    }
    catch (e) {
        ...
        err.write(`PUT 실패: ${e.message}\n`);
        return { exitCode: 1 };
    }
    const localSyncOk = await syncLocalConfig(parsed, err);
    if (localSyncOk) {
        out.write(`interval=${response.interval_seconds}s saved (web + local). ` +
            `데몬이 다음 cycle에서 자동 반영합니다.\n`);
    }
    else {
        out.write(`interval=${response.interval_seconds}s saved (web only — 로컬 sync 실패). ` +
 ...
 */
export async function runIntervalGet(out = process.stdout, err = process.stderr) {
    const config = await tryLoadConfig(err);
    if (!config)
        return { exitCode: 1 };
    let response;
    ...
    out.write(`interval=${response.interval_seconds}s ` +
...
async function tryLoadConfig(err) {
    try {
        return await loadConfig();
    }
    catch (e) {
        err.write(`config 로드 실패: ${e.message}\n`);
 ...
 */
async function syncLocalConfig(intervalSeconds, err) {
    ...
    try {
        ...
        err.write(`! 로컬 config 동기화 실패 (read): ${e.message}\n`);
        return false;
    }
    let parsed;
    try {
        ...
        err.write(`! 로컬 config 동기화 실패 (parse): ${e.message}\n`);
        return false;
    }
    ...
    try {
        ...
        err.write(`! 로컬 config 동기화 실패 (write): ${e.message}\n`);
        return false;
    }
    return true;
}
