...
import { ApiPatAuthError, getBootstrap, listEligibleProjects, putAutoUploadRooms, putSyncSettings, } from '../api-client.js';
import { bootstrapCachePath, chronosHomeDir, ensureChronosDir, saveAuth, savePatFile, } from '../auth-file.js';
import { CONFIG_FILE_NAME } from '../constants.js';
import { isKeychainAvailable, setPat } from '../keychain.js';
import { probeRunningDaemon } from '../daemon-detect.js';
export async function runMigrate(opts, io = { out: process.stdout, err: process.stderr }) {
    const out = io.out;
    const err = io.err;
    // --- step 1: daemon detection (MAJ-8.3) ---
    const probe = await probeRunningDaemon();
    if (probe.running && !opts.force) {
        err.write(`error: chronos-sync daemon is running (pids=${probe.pids.join(',') || 'none'} ` +
    ...
    try {
        ...
        err.write(`error: failed to read ${legacyPath}: ${e.message}\n`);
        return { exitCode: 1 };
    }
    ...
    const legacyServerUrl = typeof legacyParsed.server_url === 'string'
        ? legacyParsed.server_url.replace(/\/+$/, '')
        : null;
    const legacyInterval = typeof legacyParsed.interval_seconds === 'number' &&
    ...
    if (!legacyPat || legacyRoomsRaw.length === 0) {
        out.write('migrate: legacy config.json has no embedded pat/rooms — nothing to migrate.\n' +
    ...
    const skippedNoChatId = [];
    for (let i = 0; i < legacyRoomsRaw.length; i++) {
        const r = legacyRoomsRaw[i];
        ...
        else if (typeof r.chat_id === 'number' &&
    ...
    try {
        ...
        if (e instanceof ApiPatAuthError) {
            err.write('error: legacy PAT rejected by server (401). Cannot pre-flight project validation.\n' +
                '  Issue a fresh PAT and run "chronos-sync auth" directly instead.\n');
            return { exitCode: 1 };
        }
        err.write(`error: pre-flight failed: ${e.message}\n`);
        return { exitCode: 1 };
    }
    const eligibleIds = new Set(eligibleProjects.filter((p) => p.archived !== true).map((p) => p.id));
    const validRows = [];
    const droppedArchived = [];
    ...
    if (validRows.length === 0) {
        err.write(`error: no valid rooms to migrate (legacy=${legacyRoomsRaw.length}, ` +
    ...
    try {
        ...
        err.write(`error: room PUT failed: ${e.message}\n` +
    ...
    if (legacyInterval !== null) {
        try {
            ...
            err.write(`error: interval PUT failed: ${e.message}\n` +
                '  legacy config.json preserved; re-run "chronos-sync migrate" after resolving.\n');
            return { exitCode: 1 };
        }
    }
    ...
    try {
        ...
        if (bootstrap.status !== 200) {
            err.write(`error: bootstrap returned status ${bootstrap.status} unexpectedly; aborting. Legacy config preserved.\n`);
            return { exitCode: 1 };
        }
        ...
        err.write(`error: bootstrap fetch failed: ${e.message}\n` +
            '  legacy config.json preserved; re-run "chronos-sync migrate" after resolving.\n');
        return { exitCode: 1 };
    }
    ...
    if (probeKey.available) {
        ...
        err.write(`error: Keychain unavailable (${probeKey.reason ?? 'unknown reason'}).\n` +
    ...
    try {
        ...
        err.write(`! warning: auth.json + Keychain written, but renaming legacy config.json failed: ${e.message}\n` +
...
function printMigrateSummary(inputs) {
    const { out } = inputs;
