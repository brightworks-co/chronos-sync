 ...
 */
export async function runDiagnoseSenders(arg, out = process.stdout, err = process.stderr) {
    let config;
    try {
    ...
    try {
        ...
        err.write(`kakaocli messages 호출 실패: ${e.message}\n`);
        return { exitCode: 1 };
    }
    out.write(`\n=== 진단 대상 ===\n` +
...
function pickRoom(rooms, arg) {
    for (const r of rooms) {
        if (r.chat_id !== undefined && String(r.chat_id) === arg.trim())
            return r;
        if (r.chat_name !== undefined && r.chat_name === arg)
            return r;
    }
    return null;
}
function bucketSenders(messages) {
    const bucket = {
        already_resolved: 0,
        is_from_me: 0,
        unique_ids_needing_lookup: new Set(),
    };
...
async function buildIdReports(ids, messages, nameMap, binary) {
...
async function probeNTUser(senderId, binary) {
...
function printIdReports(out, reports) {
    ...
    for (const r of reports) {
        ...
        out.write(`  ${tag}  id=${r.sender_id}  msgs=${r.message_count}  ` +
            (r.detail ? `→ ${r.detail}` : '') +
            (r.sample_text ? `\n     샘플: "${r.sample_text}"` : '') +
            '\n');
        if (r.status === 'resolved')
            resolvedCount += 1;
        else if (r.status === 'fallback_absent')
            absentCount += 1;
        else
            probeFailedCount += 1;
    }
