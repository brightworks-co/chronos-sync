...
import { bootstrapCachePath, chronosHomeDir, } from './auth-file.js';
import { ApiPatAuthError, getBootstrap as fetchBootstrapHttp, } from './api-client.js';
 ...
 */
export function resetBootstrapCacheForTest() {
    cached = null;
    inFlight = null;
 ...
 */
export async function loadCachedSnapshotFromDisk() {
...
function isValidSnapshot(v) {
    return (typeof v.server_url === 'string' &&
 ...
 */
async function invalidateCacheOnDisk(log) {
    const path = bootstrapCachePath();
    const ts = Date.now();
 ...
 */
export function primeBootstrap(auth, pat, log) {
    if (inFlight)
        return inFlight;
    inFlight = (async () => {
        ...
        try {
            const result = await fetchBootstrapHttp({ serverUrl: auth.server_url, pat }, etag);
            const now = Date.now();
 ...
 */
export function getBootstrap(log, now = Date.now()) {
    if (!cached) {
        return {
            snapshot: null,
            warning: null,
 ...
 */
export function bootstrapStatusLabel(now = Date.now()) {
    if (!cached)
        return 'missing';
 ...
 */
export function peekCachedSnapshot() {
    return cached;
}
/**
 ...
 */
export function invalidatedCachePathPrefix() {
