...
export interface FeedTypePayload {
  feedType: number
  members?: Array<{ userId?: number | string; nickName?: string }>
  hidden?: boolean
 ...
 */
export function isFeedTypeText(text: string): boolean {
 ...
 */
export function parseFeedTypeText(text: string): FeedTypePayload | null {
  if (!isFeedTypeText(text)) return null
  let parsed: unknown
  try {
  ...
  if (
    ...
    typeof (parsed as { feedType?: unknown }).feedType !== 'number' ||
    !Number.isFinite((parsed as { feedType: number }).feedType)
  ) {
    return null
  }
  return parsed as FeedTypePayload
}
 ...
 */
export function feedTypeToPlaceholder(payload: FeedTypePayload): string {
 ...
 */
export function transformFeedTypeText(text: string): string {
