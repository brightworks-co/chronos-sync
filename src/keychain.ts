 ...
 */
export async function isKeychainAvailable(): Promise<KeychainProbeResult> {
  try {
    ...
    const err = e as NodeJS.ErrnoException & { stderr?: string; signal?: string }
    if (err.code === 'ENOENT') {
 ...
 */
export async function setPat(account: string, pat: string): Promise<void> {
  if (!account || !pat) {
 ...
 */
export async function getPat(account: string): Promise<string | null> {
  if (!account) {
  ...
  try {
    ...
    const err = e as { stderr?: string; stdout?: string; code?: string | number }
...
export async function deletePat(account: string): Promise<void> {
  ...
  try {
    ...
    const err = e as { stderr?: string; code?: string | number }
    const stderr = (err.stderr ?? '').toString()
    if (
      stderr.includes('could not be found') ||
      err.code === 44 ||
