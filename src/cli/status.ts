...
interface StatusInputs {
  ...
  configPath: string
  statePath: string
  config: DaemonConfig | { error: string }
  state: DaemonState
  now: number
}
...
export function renderStatus(inputs: StatusInputs): string {
  const lines: string[] = []
...
function buildRow(room: RoomConfig, state: DaemonState, now: number): Row {
...
export function formatLastSync(lastSuccessAt: number, now: number): string {
  if (!lastSuccessAt || lastSuccessAt <= 0) return 'never'
...
export async function runStatus(out: NodeJS.WritableStream = process.stdout): Promise<void> {
  let config: DaemonConfig | { error: string }
  try {
