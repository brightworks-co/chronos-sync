 ...
 */
export function parseHarvestArgs(args: ReadonlyArray<string>): HarvestCliOptions {
  const out: HarvestCliOptions = {}
  for (let i = 0; i < args.length; i++) {
    const a = args[i]
    switch (a) {
      case '--top': {
        const v = Number(args[++i])
        if (!Number.isFinite(v) || v < 0) throw new Error('--top requires a non-negative number')
...
export async function runHarvest(
  ...
  let opts: HarvestCliOptions
  try {
    opts = parseHarvestArgs(argv)
  } catch (err) {
    io.stderr.write(
      'chronos-sync harvest: ' + (err instanceof Error ? err.message : String(err)) + '\n'
    )
  ...
  try {
    ...
    io.stderr.write(
      'chronos-sync harvest: config load error: ' +
  ...
  try {
    ...
    if (!caps.scrollSupported) {
      io.stderr.write(
        `chronos-sync harvest: kakaocli (${binary})는 --scroll을 지원하지 않습니다. kakaocli를 업그레이드하세요.\n`
      )
      return { exitCode: 1, code: -1, stderr: '' }
    }
  } catch (err) {
    io.stderr.write(
      `chronos-sync harvest: probe 실패: ${err instanceof Error ? err.message : String(err)}\n`
    )
    return { exitCode: 1, code: -1, stderr: '' }
  }

  io.stdout.write(
    'KakaoTalk을 foreground로 두고 마우스/키보드를 사용하지 마세요. harvest 진행 중...\n'
  )
  ...
  io.stderr.write(
    `harvest 실패 (exit ${result.code}). stderr: ${result.stderr.slice(0, 500)}\n`
  )
