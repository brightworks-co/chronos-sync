...
export type ParseAuthArgsResult =
  | { kind: 'options'; options: AuthCliOptions }
  | { kind: 'help' }
  | { kind: 'invalid'; message: string }

export function parseAuthArgs(argv: readonly string[]): ParseAuthArgsResult {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a.startsWith('--token=')) {
      ...
      if (!value) return { kind: 'invalid', message: '--token requires a value' }
      options.token = value
      options.tokenWasFlag = true
      continue
    }
