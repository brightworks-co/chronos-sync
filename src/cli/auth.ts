...
export async function runAuth(
  opts: AuthCliOptions,
  io: AuthCliIo = { out: process.stdout, err: process.stderr }
  ...
  if (legacy) {
    err.write(
      'error: Legacy config.json detected with embedded credentials.\n' +
  ...
  if (opts.reset) {
    ...
    if (resetResult.exitCode !== 0) return resetResult
  }
  ...
  if (!PAT_REGEX.test(pat)) {
    err.write(
      'error: PAT format invalid — expected "chr_pat_" followed by 32 hex characters.\n' +
  ...
  let userEmail: string | null = bootstrap.kind === 'ok' ? bootstrap.payload.user_email : null
  if (!userEmail) {
    ...
    err.write(
      'error: Bootstrap fetch failed and no prior auth.json exists, so user_email is unknown.\n' +
  ...
  try {
    ...
    err.write(`error: failed to persist PAT (${storage}): ${(e as Error).message}\n`)
    return { exitCode: 1 }
  }
  ...
  try {
    ...
    err.write(`error: failed to write auth.json: ${(e as Error).message}\n`)
    return { exitCode: 1 }
  }
  ...
  if (bootstrap.kind === 'ok') {
    try {
      ...
      err.write(`! warning: failed to write config.cache.json: ${(e as Error).message}\n`)
    }
  }
  ...
  if (bootstrap.kind === 'ok') {
    out.write(
      `auth saved (rooms: ${bootstrap.payload.rooms.length}, ` +
 ...
 */
async function detectLegacyConfig(): Promise<boolean> {
  ...
  if (typeof v.pat === 'string' && v.pat.length > 0) return true
  if (Array.isArray(v.rooms) && v.rooms.length > 0) return true
  return false
}
...
interface FetchUnreachable {
  kind: 'unreachable'
}
interface FetchAuthFailed {
  kind: 'auth-failed'
}
type FetchOutcome = FetchOk | FetchUnreachable | FetchAuthFailed
 ...
 */
async function fetchBootstrapWithBackoff(
  opts: { serverUrl: string; pat: string },
  err: NodeJS.WritableStream
  ...
  for (let i = 0; i < delays.length; i++) {
    try {
      ...
      if (isLast) {
        err.write(
          `! warning: bootstrap fetch failed (${(e as Error).message}). ` +
...
async function writeBootstrapCache(payload: BootstrapPayload, etag: string): Promise<void> {
...
async function acquirePat(opts: AuthCliOptions, io: AuthCliIo): Promise<string> {
  if (opts.token) {
    if (opts.tokenWasFlag) {
      io.err.write(
        '! warning: --token <PAT> exposes the PAT in shell history; ' +
 ...
 */
async function defaultPromptHidden(prompt: string): Promise<string> {
 ...
 */
async function runReset(_opts: AuthCliOptions, io: AuthCliIo): Promise<AuthCliResult> {
  const out = io.out
  const err = io.err
  ...
  try {
    ...
    err.write(
      `! warning: could not read old PAT for unregister (${(e as Error).message}). ` +
    ...
    try {
      ...
      if (result.status === 200) {
        ...
        for (const room of rooms) {
          try {
            await deleteAutoUploadRoom(
              { serverUrl: existing.server_url, pat: oldPat },
              room.project_id,
              room.room_name
            )
            ...
            err.write(
              `! warning: failed to release ${room.project_id}/${room.room_name}: ` +
                `${(e as Error).message}\n`
            )
          }
        }
      ...
      if (e instanceof ApiPatAuthError) {
        err.write(
          '! warning: old PAT already invalid (401); cannot auto-unregister rooms.\n' +
        ...
        err.write(`! warning: unregister bootstrap fetch failed: ${(e as Error).message}\n`)
      }
    }
  }
  ...
  try {
    ...
    err.write(`! warning: failed to delete Keychain entry: ${(e as Error).message}\n`)
  }
  try {
    await wipeAuth()
  } catch (e) {
    err.write(`! warning: failed to wipe auth.json/auth.token: ${(e as Error).message}\n`)
  }
  ...
  try {
    ...
    if (errCode !== 'ENOENT') {
      err.write(`! warning: failed to remove cache: ${(e as Error).message}\n`)
    }
  }
